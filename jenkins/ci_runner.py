...
def zone1_start(cfg: dict[str, Any], term: Terminal | None) -> int:
    ...
    if cfg["arch"] == "x86_64":
        def run_x86_raw(command: str, timeout: float = 15.0) -> str:
            return run_and_print_quiet_raw(
                term,
                command,
                quiet_seconds=1.0,
        ...
        boot_log = "/tmp/boot_zone1.log"
        run_x86_raw("cd /root")
        for inspection_command in ("ls", "cat boot_zone1.sh"):
            run_x86_raw(inspection_command)
        run_x86_raw(f"sh ./boot_zone1.sh >{boot_log} 2>&1 &")
        boot_rc = 0
        time.sleep(5.0)
        run_x86_raw(f"cat {boot_log}")
        run_x86_raw("./hvisor zone list")
    else:
    ...
    if not pts_numbers:
        if cfg["arch"] == "x86_64":
            run_x86_raw("printf 'x86 secure-isolation guest pts unavailable; continuing after boot log\\n'")
            if boot_rc != 0:
