...
def zone1_start(cfg: dict[str, Any], term: Terminal | None) -> int:
    ...
    is_x86_qemu = cfg["arch"] == "x86_64"
    if is_x86_qemu:
        raw_startup_plan = [
            ("cd /root", 1.0, 15.0),
            ("ls", 1.0, 15.0),
            ("cat boot_zone1.sh", 1.0, 15.0),
            ("sh ./boot_zone1.sh >/tmp/boot_zone1.log 2>&1 &", 1.0, 15.0),
        ]
        for command, quiet_seconds, max_duration in raw_startup_plan:
            _ = run_and_print_quiet_raw(
                term,
                command,
                quiet_seconds=quiet_seconds,
                max_duration=max_duration,
        ...
        boot_rc = 0
        time.sleep(5.0)
        for command in ("cat /tmp/boot_zone1.log", "./hvisor zone list"):
    ...
    if not pts_numbers:
        if is_x86_qemu:
            print("[ci_runner] x86 zone boot produced no attachable guest pts; skipping screen probe", flush=True)
            if boot_rc != 0:
