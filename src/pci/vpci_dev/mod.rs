use crate::pci::pci_struct::{ArcRwLockVirtualPciConfigSpace, Bdf, CapabilityType, ConfigValue, PciCapabilityRegion, VirtualPciConfigSpace};
...
impl PciCapabilityRegion for VirtMsiCap {
    fn read(&self, offset: PciConfigAddress, size: usize) -> HvResult<u32> {
        match (offset, size) {
            (0x0, 2) => Ok((CapabilityType::Msi.to_id() as u32) | (self.next_cap_pointer << 8) as u32),
            ...
            _ => {
                warn!("VirtMsiCap invalid read offset 0x{:x} size {}", offset, size);
                Ok(0)
            }
        }
    }
    fn write(&mut self, offset: PciConfigAddress, size: usize, value: u32) -> HvResult {
        match (offset, size) {
            ...
            _ => {
                warn!("VirtMsiCap invalid write offset 0x{:x} size {}", offset, size);
            }
        }
        Ok(())
    }
...
impl PciCapabilityRegion for VirtMsiXCap {
    fn read(&self, offset: PciConfigAddress, size: usize) -> HvResult<u32> {
        match (offset, size) {
            (0x0, 4) => Ok(((CapabilityType::MsiX.to_id() as u32) | (self.next_cap_pointer << 8) as u32) | (((self.control_bits.as_raw_slice()[0] as u32) << 16) as u32)),
            (0x0, 2) => Ok((CapabilityType::MsiX.to_id() as u32) | (self.next_cap_pointer << 8) as u32),
            (0x2, 2) => Ok(self.control_bits.as_raw_slice()[0] as u32),
            (0x4, 4) => Ok(self.table),
            (0x8, 4) => Ok(self.pba),
            _ => {
                warn!("VirtMsiXCap invalid read offset 0x{:x} size {}", offset, size);
                Ok(0)
            }
        }
    }
    fn write(&mut self, offset: PciConfigAddress, size: usize, value: u32) -> HvResult {
        match (offset, size) {
            ...
            _ => {
                warn!("VirtMsiXCap invalid write offset 0x{:x} size {}", offset, size);
            }
        }
