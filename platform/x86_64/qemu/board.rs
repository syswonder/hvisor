...
// Generated by build.rs: `ASTERINAS_ROOT_INITRD_SIZE` is the byte length of the
// staged uncompressed initramfs when the `asterinas_root` feature is enabled,
// 0 otherwise.
include!(concat!(env!("OUT_DIR"), "/asterinas_initrd.rs"));
...
#[cfg(not(feature = "asterinas_root"))]
pub const ROOT_ZONE_NAME: &str = "root-linux";
#[cfg(feature = "asterinas_root")]
pub const ROOT_ZONE_NAME: &str = "root-asterinas";

// The default x86_64/qemu root zone is Linux. Building with the
// `asterinas_root` feature (e.g.
// `make ARCH=x86_64 BOARD=qemu FEATURES="pci ecam_pcie no_pcie_bar_realloc uart16550a intel_vtd asterinas_root"`)
// instead boots Asterinas as the root zone: the command line below is selected
// and a multiboot2 initramfs is passed through ROOT_ARCH_ZONE_CONFIG. Boot the
// "Hvisor (Asterinas root zone)" GRUB entry for this build.
#[cfg(not(feature = "asterinas_root"))]
...
#[cfg(feature = "asterinas_root")]
pub const ROOT_ZONE_CMDLINE: &str =
    "SHELL=/bin/sh LOGNAME=root HOME=/ USER=root PATH=/bin:/benchmark ostd.log_level=info console=ttyS0 -- sh -l\0";
...
pub const ROOT_ZONE_MEMORY_REGIONS: [HvConfigMemoryRegion; 11] = [
    ...
    HvConfigMemoryRegion {
        ...
        size: 0xc0_0000,
    }, // pci mmio bar window (below ioapic/hpet)
];
...
// Root-zone initramfs. Linux here uses a virtio-blk rootfs, so no boot-params
// ramdisk is set; the Asterinas root zone is delivered as an uncompressed
// multiboot2 initramfs staged at GPA 0x1530_0000 (HPA 0x1a30_0000, see
// grub.cfg), with its true byte length captured by build.rs.
#[cfg(not(feature = "asterinas_root"))]
const ROOT_ZONE_INITRD_LOAD_GPA: GuestPhysAddr = 0;
#[cfg(feature = "asterinas_root")]
const ROOT_ZONE_INITRD_LOAD_GPA: GuestPhysAddr = 0x1530_0000;
#[cfg(not(feature = "asterinas_root"))]
const ROOT_ZONE_INITRD_SIZE: usize = 0;
#[cfg(feature = "asterinas_root")]
const ROOT_ZONE_INITRD_SIZE: usize = ASTERINAS_ROOT_INITRD_SIZE;
...
pub const ROOT_ARCH_ZONE_CONFIG: HvArchZoneConfig = HvArchZoneConfig {
    ...
    initrd_load_gpa: ROOT_ZONE_INITRD_LOAD_GPA,
...
// The q35 MCFG base is 0xb0000000. The 2 MiB ECAM window (ecam_size 0x200000)
// is mapped for the populated buses 0..=1 (all ROOT_PCI_DEVS live there;
// unpopulated buses read 0xffff). `bus_range_end` stays 0x1f as the vpci scan
// bound, intentionally NOT narrowed to the window size: narrowing it to a
// bridge subordinate bus can make the vpci iterator stop scanning the root bus
// early. hvisor rewrites the guest-visible ACPI MCFG bus range to
// ROOT_PCI_MAX_BUS in src/arch/x86_64/acpi.rs.
