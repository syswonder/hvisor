...
// staged uncompressed initramfs when `CONFIG_ASTERINAS_ROOT` is enabled,
// 0 otherwise.
...
#[cfg(not(asterinas_root))]
pub const ROOT_ZONE_NAME: &str = "root-linux";
#[cfg(asterinas_root)]
pub const ROOT_ZONE_NAME: &str = "root-asterinas";

// The default x86_64/qemu root zone is Linux. Building with the
// `ASTERINAS_ROOT` Kconfig option enabled (e.g. `make ARCH=x86_64 BOARD=qemu
// defconfig` then `sed -i 's/^# CONFIG_ASTERINAS_ROOT is not set$/CONFIG_ASTERINAS_ROOT=y/' .config`)
